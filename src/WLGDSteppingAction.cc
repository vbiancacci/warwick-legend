                    ...
                    if(aStep->GetPostStepPoint()->GetGlobalTime() / us < 10.)
                    {
                        if(aStep->GetPostStepPoint()
                            ...
                            if(fRunAction->getWriteOutAdvancedMultiplicity()){
                                if(std::find(fEventAction->GetIDListOfGdSiblingParticles().begin(), fEventAction->GetIDListOfGdSiblingParticles().end(), aStep->GetTrack()->GetParentID()) != fEventAction->GetIDListOfGdSiblingParticles().end())
                    ...
                    else
                    {
                        if(aStep->GetPostStepPoint()->GetGlobalTime() / ms < 1.)
                        {
                            if(aStep->GetPostStepPoint()
                                ...
                                if(fRunAction->getWriteOutAdvancedMultiplicity()){
                                    if (std::find(fEventAction->GetIDListOfGdSiblingParticles().begin(),
                                                  fEventAction->GetIDListOfGdSiblingParticles().end(), aStep->GetTrack()->GetParentID()) !=
                                        fEventAction->GetIDListOfGdSiblingParticles().end())
