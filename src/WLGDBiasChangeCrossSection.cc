...
void WLGDBiasChangeCrossSection::StartRun()
{
  ...
  if(fSetup)
  {
                               ...
                               // processes.
    {
      ...
      {
        // G4cout << "XSchange-" << wrapperProcess->GetWrappedProcess()->GetProcessName()
        // << G4endl;
  ...
  const G4Track* track, const G4BiasingProcessInterface* callingProcess)
{
  ...
  else
  {
    if(track->GetParticleDefinition()->GetParticleName() == "neutron")
    {
      if(callingProcess->GetWrappedProcess()->GetProcessName() == "nCapture")
      {
        if(track->GetStep()
          ...
          XStransformation =
            fNeutronBias *
            1.68;  // specific for this, boost n,gamma by 68% for 77Ge from 76Ge
        else
    ...
    else if(track->GetParticleDefinition()->GetParticleName() == "pi+")
    {
      XStransformation = fNeutronYieldBias;
    }
    else if(track->GetParticleDefinition()->GetParticleName() == "pi-")
    ...
    else if(track->GetParticleDefinition()->GetParticleName() == "proton")
    {
      XStransformation = fNeutronYieldBias;
    }
    else if(track->GetParticleDefinition()->GetParticleName() == "kaon-")
    {
      XStransformation = fNeutronYieldBias;
    }
    else
    {
      XStransformation = 1.0;
    }
  }
  // G4cout << "XStransformation: " << XStransformation << " | " << fpname << " - " <<
  // callingProcess->GetWrappedProcess()->GetProcessName() << " - " << XStransformation *
  // analogXS << G4endl;
  ...
  if(previousOperation == nullptr ||
     previousOperation != operation /*&& fNeutronYieldBias != 1.)*/)
  {
    operation->SetBiasedCrossSection(XStransformation * analogXS);
    operation->Sample();
  }
  ...
  {
    //      G4cout << "operation->GetInteractionOccured(): " <<
    //      operation->GetInteractionOccured() << " - previousOperation: " <<
    //      previousOperation->GetName() << " " << previousOperation << " - operation: "
    //      << operation->GetName() << " " << operation << G4endl;
    if(previousOperation != operation)
    {
      // -- should not happen !
      G4ExceptionDescription ed;
      ed << " Logic problem in operation handling ! - previousOperation: "
         << previousOperation->GetName() << " " << previousOperation
         << " - operation: " << operation->GetName() << " " << operation << G4endl;
