...
void WLGDDetectorConstruction::DefineMaterials() {
    ...
    G4double density =
            3.01 * g / cm3; //https://www.sigmaaldrich.com/catalog/product/aldrich/203300?lang=de&region=DE room temp
...
auto WLGDDetectorConstruction::SetupBaseline() -> G4VPhysicalVolume * {
    ...
    auto *fTankLogical = new G4LogicalVolume(tankSolid, steelMat, "Tank_log");
    auto *fTankPhysical =
            new G4PVPlacement(nullptr, G4ThreeVector(0., 0., /*-stone -offset_2*/ -(hallhheight - tankhheight) * cm),
                              fTankLogical,
                              "Tank_phys", fHallLogical, false, 0, true);
    ...
    //
    // ULAr bath, solid cylinder
    auto *ularSolid = new G4Tubs("ULar", 0.0 * cm, (curad - copper) * cm, (cuhheight - copper) * cm,
                                 0.0, CLHEP::twopi);
...
void WLGDDetectorConstruction::SetBoratedPET(G4int answer) {
...
// option to set the radius of the turbine structure
    fDetectorMessenger
            ->DeclareMethod("Borated_Turbine_Length", &WLGDDetectorConstruction::SetBoratedTurbineLength)
